...
def run_training(args, tune_config={}, reporter=None):
    ...
    for i in range(args.start_iter, args.iters):
        ...
        # NOTE: The original code used the deprecated .reinforce() method from PyTorch < 1.0
        # This RL training approach is deprecated and may not work with modern PyTorch
        # For a proper implementation, consider using policy gradient with log probabilities
        # Keeping original structure for reference, but this will need refactoring for modern PyTorch
        policy_losses = []
        for action, R in zip(gate_saved_actions, cum_rewards):
            # Approximate the original reinforce() behavior
            # Original: action.reinforce(args.rl_weight * R)
            if hasattr(action, 'reinforce'):
                action.reinforce(args.rl_weight * R)
            else:
                # For newer PyTorch, accumulate as policy loss (requires proper log_prob tracking in models)
                policy_losses.append(-action * args.rl_weight * R)

        if policy_losses:
            total_loss = total_criterion(output, target) + sum(policy_losses).mean()
        else:
            total_loss = total_criterion(output, target)

        optimizer.zero_grad()
        # optimize hybrid loss
        if not policy_losses and gate_saved_actions:
            # Old style with reinforce() 
